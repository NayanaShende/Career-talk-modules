...
const getImageUri = (image, fallbackName) => {
  if (image) {
  ...
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(fallbackName || "User")}&background=1A2B4C&color=fff`;
};
...
const ReviewCard = ({ review }) => {
  ...
  // ✅ FIXED: use smart getImageUri instead of always prepending /uploads/
  const avatarUri = getImageUri(
    userImage,
    userName,
  );
      ...
      >
        ...
        <View style={styles.profileHeader}>
          <View style={styles.avatarWrapper}>
            {/* ✅ FIXED: use smart getImageUri instead of always prepending /uploads/ */}
            <Image
              source={{ uri: getImageUri(expert.image, expert.name || "Expert") }}
              style={styles.avatar}
              onError={(e) => {
                e.currentTarget.setNativeProps({
                  src: [
