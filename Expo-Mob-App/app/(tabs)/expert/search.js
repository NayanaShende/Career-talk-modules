...
export default function Home() {
  ...
  // ✅ FIXED: Smart image URL — checks expert.image first,
  //           then falls back to user.image, then ui-avatars
  const getImageUri = (item) => {
    ...
    // ✅ Priority 2: user image from association (if included in API response)
    const userImage = item?.user?.image;
    if (userImage) {
      if (userImage.startsWith("http://") || userImage.startsWith("https://")) {
        return userImage;
      }
      const clean = userImage.replace(/^uploads\//, "");
      return `${BASE_URL}/uploads/${clean}`;
    }

    // ✅ Priority 3: ui-avatars fallback with expert's initials
    const name = item?.name || "Expert";
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=0B2D72&color=fff&size=200&bold=true`;
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            // ✅ FIXED: pass full item object for fallback chain
