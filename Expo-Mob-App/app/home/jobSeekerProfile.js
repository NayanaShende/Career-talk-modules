...
const JobseekerProfileScreen = () => {
  ...
  const handleFilePick = async () => {
    try {
      ...
      if (result.type === "success") {
        setFormData({ ...formData, cvFile: result });
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (formData.cvFile) {
        payload.append("cvFile", {
          uri: formData.cvFile.uri,
          name: formData.cvFile.name,
          type: formData.cvFile.mimeType || "application/pdf",
        });
      }
      ...
      Alert.alert("Success", "Jobseeker Profile Created!");
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.fileBtn} onPress={handleFilePick}>
        <Text style={styles.fileBtnText}>
          {formData.cvFile ? `Selected: ${formData.cvFile.name}` : "Upload CV"}
        </Text>
      </TouchableOpacity>
